...
func init() {
	android.RegisterModuleType("cygnus_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = cygnusExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = cygnusExpandVariables(ctx, dep_file)
	...
	exCmd := cygnusExpandVariables(ctx, String(g.properties.Cmd))
	...
	manifestName := "cygnus.sbox.textproto"
	desc := "generate"
	name := "generator"
