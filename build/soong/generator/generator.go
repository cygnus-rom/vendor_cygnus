...
func init() {
	android.RegisterModuleType("rockstar_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = rockstarExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = rockstarExpandVariables(ctx, dep_file)
	...
	cmd := rockstarExpandVariables(ctx, String(g.properties.Cmd))
